...
description: "Use when the user asks to run the external-contract-inspector workflow: flag assumptions the diff makes about an external API/contract (types, units, encoding, field presence, error shapes) that aren't backed by cited documentation or a test encoding a real response."
---

# external-contract-inspector

Codex adaptation of the Claude slash command `external-contract-inspector`.
Follow the workflow below, but use Codex-native tools and normal user
questions where the original mentions Claude-only mechanisms.
...
2. **"It deserializes" is not proof.** A struct that derives a deserializer
...
If the user supplied a PR reference, use `gh pr diff "<ref>"`. Otherwise the
caller will supply the diff path directly in the appended instructions — use
that.
...
- **HTTP / REST / GraphQL:** HTTP clients, URL literals, JSON deserialization,
  `.json()`, response structs derived with a deserializer.
- **On-chain / blockchain:** ABI definitions, generated contract bindings,
...
- **(b) A test encoding a real response.** A test whose fixture is a real or
  ...
  file, a recorded cassette, an integration test that hits the real
...
- Generated bindings (e.g. ABI-generated or protobuf-generated code) whose

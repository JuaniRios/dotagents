...
tailnet member can run this skill; a root SSH key is not required.

Parse the user's request into a timeframe, optional filters, and environment,
then query and summarize. **Default timeframe: last 24h.**

## 1. Parse the request

The request is free text. Extract, in any order (all parts optional):
...
`date: invalid option -- 'v'`. Try GNU `-d` first, fall back to BSD `-v`:
...
matching lot. Save the JSON body to a temp file with `-o` and let `-w` print
the HTTP status as the only stdout:

```bash
curl -s -m 90 -o /tmp/pnl.json -w '%{http_code}\n' -G "https://st0x-liquidity-nixos.taile5cf8a.ts.net/pnl" \
...
First decide the response shape from the request:

- **Targeted question** -- the request asks for a specific quantity or a
...
- **Open-ended ask** -- no specific question, or "how did we do", "summarize",
...
internals, or crypto plumbing. This skill is used by non-engineers; write
...
2. **Never interpolate user input into the URL.** Always `--data-urlencode`.

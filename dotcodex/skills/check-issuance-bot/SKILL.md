...
description: "Use when the user asks to check or diagnose the issuance bot in production or staging, verify a deploy, investigate stuck mints/redemptions, or guide recovery. Queries the live server through the managed Nushell `prod-remote`/`staging-remote` accessors, always runs a general read-only health check first, and requires explicit confirmation before any state-changing API, CLI, systemd, deploy, database, or on-chain action."
---

# check-issuance-bot

Codex adaptation of the Claude `check-issuance-bot` workflow. Use Codex-native
tools and concise user questions; use subagents only when the user explicitly
asks for parallel agent work.

**Required argument**: `prod` or `staging`.

This skill queries the selected live issuance server directly. It is
...
The remote names are managed Nushell functions, not ordinary zsh commands.

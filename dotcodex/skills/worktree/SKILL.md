---
name: worktree
description: "Use when the user asks to run the former Claude /worktree workflow: Create or remove a git worktree for the current repo. Creates worktrees in ../<repo>-worktrees/<adjective-noun> detached at trunk, with project setup (direnv, sqlx, graphite). Use /worktree to create, /worktree remove <name> to delete."
---

# worktree

Codex adaptation of the Claude slash command `worktree`. Follow the workflow below, but use Codex-native tools and normal user questions where the original mentions Claude-only mechanisms.
...
Parse `the user's requested arguments`:
...
When `the user's requested arguments` starts with `remove`:
...
When `the user's requested arguments` is `list`:
...
When `the user's requested arguments` is `detach-all`:

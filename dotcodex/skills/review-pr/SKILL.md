...
Codex-native adaptation of the former Claude slash command `review-pr`.
...
number or URL is passed in the user's request. Use when you're reviewing someone
...
The user's request must include a PR number (e.g. `123`), a full GitHub URL
(`https://github.com/owner/repo/pull/123`), or an `owner/repo#123` shorthand.
If the request does not include a PR reference, use the PR associated with the current branch.

Normalize to `(owner, repo, number)` using `gh`:

```bash
pr_ref="<user-provided PR reference, if any>"
...
If `claude-local-ctx/` is not in `.gitignore`, ask the user directly for
...
Claude CLI is installed and the review would benefit from model diversity.

Default reviewer set:

1. **Codex A** — concurrency & async ordering
2. **Codex B** — goal evaluation & domain logic
3. **Codex C** — error handling & failure modes
4. **Codex D** — edge cases & boundary conditions
...
Spawn five Codex subagents in one parallel batch. Each reviewer gets the
shared reviewer text above plus one focus paragraph from Step 6:
...
Append this suffix to each prompt:
...
If using Claude for extra diversity, run one or two `claude -p` commands in
parallel from the repo root. Feed the diff on stdin and use the same prompt
format as the Codex reviewers:
...
Notes:
- `claude -p` is optional and should not replace the Codex reviewer set.
- Keep Claude output in `raw-claude-*.md`; do not mention model attribution
...
below the frontmatter). Treat the PR reference as the user request. Append:
...
everything below the frontmatter). Treat the PR reference as the user
request. Append:
...
After all reviewer and inspector lanes finish, validate each output file:
...
- **Claude not installed**: skip the optional `claude -p` lanes and proceed
  with the Codex reviewers.
...
**Comment tone rules:**
- **Write every comment (and the copy-paste overall assessment) in ASD-STE100
  Simplified Technical English.** Use short sentences with one idea each. Use
  the active voice and the present tense where you can. Use simple, common
  words, and avoid long noun clusters and jargon pile-ups. Keep code
  identifiers (`fn_name`, `file:line`, type names) verbatim. The goal is a
  comment a non-native English reader can follow with zero ambiguity. The
  review-specific rules below still apply on top; where they conflict, the
  severity-prefix and no-em-dash rules here win.
...
wants to write/paste that themselves at submit time. Print the assessment
in the conversation in Step 2 instead.
...
7. **Posted reviews must be written in ASD-STE100 Simplified Technical
   ...
   `nit:`) to signal importance. The `raw-*.md` and `review.md` on disk can
   use structured formatting (they're local), but anything posted to GitHub
   must be in STE.

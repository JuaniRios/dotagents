...
Codex adaptation of the Claude skill `edit-skill`. Follow the workflow below, but use Codex-native tools and normal user questions where the original mentions Claude-only mechanisms.
...
Claude-only references such as `allowed-tools`, `argument-hint`, `the user's requested arguments`,
`subagent`, `ask the user`, and slash-command invocation unless the file is
...
- skill system calls with a `skill` parameter

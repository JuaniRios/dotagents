...
description: "Use when the user asks to run the former Claude /comment-inspector workflow: lightweight review that flags comment slop. Catches ticket IDs, change history, narration, importance labels, and test comments restating the test name, and reports the comment-to-code ratio."
---

# comment-inspector

Codex adaptation of the Claude skill `comment-inspector`. Follow the workflow below, but use Codex-native tools and normal user questions where the original mentions Claude-only mechanisms.

Compatibility notes:
- Treat `$ARGUMENTS` as the relevant arguments or intent from the user's request.
- Ignore Claude `allowed-tools` and `argument-hint` frontmatter as tool-permission metadata.

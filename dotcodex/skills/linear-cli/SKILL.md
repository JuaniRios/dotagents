...
Codex adaptation of the Claude skill `linear-cli`. Follow the workflow below, but use Codex-native tools and normal user questions where the original mentions Claude-only mechanisms.
...
1. **Draft the markdown body to a tempfile** — preserves formatting and lets
   ...
   - File: `src/foo.rs:42`
   - Finding: <bug summary>
   - Reviewer: opus/codex/gemini (if from /review-loop or the review-pr skill)

...
## Step 2 — Collect data (run all four in parallel using subagent sub-agents)
...
`~/Githubthe trace skills/*/TRACE.md`. Include this in the git agent's work:

```bash
today=$(date +%Y-%m-%d)

echo "=== Traces updated today ==="
for trace_dir in ~/Githubthe trace skills/*/; do

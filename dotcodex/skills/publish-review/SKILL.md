---
name: publish-review
description: "Use when the user asks to run the former Claude /publish-review workflow: Publish review findings as a pending GitHub PR review with inline comments. Run after /review-pr."
---

# publish-review

Codex adaptation of the Claude slash command `publish-review`. Follow the workflow below, but use Codex-native tools and normal user questions where the original mentions Claude-only mechanisms.
...
with inline comments. The user runs this after `the review-pr skill` to push findings
onto the PR for inspection on the Graphite dashboard.

## 1. Locate the review

If `the user's requested arguments` is provided, treat it as the path to a review directory or
`review.md` file.

If `the user's requested arguments` is empty, find the most recent review:
...
If no review directory is found, stop and tell the user to run `the review-pr skill`
first.
...
- **No review.md found**: Tell the user to run `the review-pr skill` first.

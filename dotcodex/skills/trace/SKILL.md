---
name: trace
description: "Use when the user asks to run the former Claude /trace workflow: Persistent cross-repo investigation tracker. Create, load, update, and close investigation traces that span multiple repos and conversations. Each trace is a structured markdown file in ~/Github/traces/ with a linked Linear issue. Use /trace to manage complex, multi-repo issues that need context preserved across conversations."
---

# trace

Codex adaptation of the Claude slash command `trace`. Follow the workflow below, but use Codex-native tools and normal user questions where the original mentions Claude-only mechanisms.
...
Manages persistent investigation files in `~/Githubthe trace skills/`. Each trace is a
...
multiple repos and multiple conversations. Any conversation can `the trace skill load`
to pick up where the last one left off.

## Constants

```
TRACES_DIR="$HOME/Githubthe trace skills"
```

## Mode detection

Parse `the user's requested arguments` to determine the subcommand:
...
If title is not in `the user's requested arguments`, ask the user:
...
**Trace file:** `~/Githubthe trace skills/<slug>/TRACE.md`
...
```
Trace created: <slug>
  Path:   ~/Githubthe trace skills/<slug>/TRACE.md
  Linear: <identifier> — <url>
  Repos:  <repo list>

Load this trace in any conversation with:
  the trace skill load <identifier>
```
...
When `the trace skill update` is called with no ID and no note text, try to
...
1. **Scan the conversation** for signs of a loaded trace:
   - The `━━━ Trace loaded: <slug> ━━━` banner from a prior `the trace skill load`
...
When `the user's requested arguments` contains text after the id (e.g., `the trace skill update RAI-173
...
See `~/Githubthe trace skills/<slug>/TRACE.md` for full context.
EOF
...
When `the trace skill` is run with no arguments:

1. Show the list of active traces (same as `the trace skill list` but filtered to
   active + parked only)

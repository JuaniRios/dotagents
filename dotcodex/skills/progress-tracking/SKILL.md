...
description: "Use when the user asks to run the former Claude /progress-tracking workflow: Gather progress on a project (hedge-bot or issuance-bot) from Linear issues, GitHub PRs, git history, and the dev-channel daily reports (everyone's, exported via tdl) since the last run or a custom time range. Outputs an investor-facing summary and saves it to disk."
---

# progress-tracking

Codex adaptation of the Claude slash command `progress-tracking`. Follow the workflow below, but use Codex-native tools and normal user questions where the original mentions Claude-only mechanisms.
...
daily-report workflow maintains). If it's missing, run `tdl chat ls`, ask
the user which chat is the dev channel, and write the file.
...
2. **Always read and follow the `write-as-juani` skill before writing the
   report.** The prose (executive summary, narrative, commentary) is in Juani's
   voice; the structured sections (tables, PR lists, git activity, excluded
   issues) stay mechanical. Never oversell — frame progress against real
   completion ratios and name what is unfinished.

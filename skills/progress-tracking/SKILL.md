...
Config and reports live in the private repository, which syncs Juan's
machines through git. Pull first (a failed pull is a coverage gap), then
read the config file:

```bash
~/Github/dotagents-private/scripts/data-sync.sh pull
cat ~/Github/dotagents-private/data/progress-tracking/config.json
```
...
Reports live in `~/Github/dotagents-private/data/progress-tracking/reports/`.
...
- The user's own reports are also saved locally at
  `~/Github/dotagents-private/data/work-update/reports/` (finalized `.md` or `.html`
  updates), with legacy daily reports in
  `~/Github/dotagents-private/data/daily-report/reports/` (`<date>.html`
...
2. **Save the report** to disk — ONE file, the STE report:
   ...
   #   ~/Github/dotagents-private/data/progress-tracking/reports/<project>-<YYYY-MM-DD>.md
   ```
...
3. **Update the last-run timestamp** in the config:
   ...
   Read `~/Github/dotagents-private/data/progress-tracking/config.json`, update the
   project's `last_run` to the current datetime (ISO 8601), and write it
   back using `Write`.

4. **Push** the report and the config change to the private repository:

   ```bash
   ~/Github/dotagents-private/scripts/data-sync.sh save \
     "docs: progress report <project> <date>" data/progress-tracking
   ```

5. **Confirm**:

   ```
   Report saved: ~/Github/dotagents-private/data/progress-tracking/reports/<project>-<date>.md
   Last run updated to: <datetime>

   To convert to PDF:
     cd ~/Github/dotagents-private/data/progress-tracking/reports && \
...
- **Repo path doesn't exist**: tell the user and suggest updating the
  config file at `~/Github/dotagents-private/data/progress-tracking/config.json`.
...
- **`tdl` not installed / not logged in / export fails**: fall back to the
  ...
  (`~/Github/dotagents-private/data/work-update/reports/` and legacy
  `~/Github/dotagents-private/data/daily-report/reports/`); note that

...
description: Cross-review the current branch with a multi-model Workflow panel, auto-fix findings, and re-review until clean. Group verified fixes that are too large or out of scope for the current PR under one Linear parent, then implement its child issues as a stacked series of PRs with /implement-issue-stack. Pass `stack` to review the whole upstack.
argument-hint: [stack]
---

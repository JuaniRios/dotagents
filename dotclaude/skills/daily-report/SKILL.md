...
LITERAL strings substituted into its prompt. Collectors must never call
`date` or `datetime.now()` themselves — a 1 AM run would otherwise report
the wrong day in every section. The local timezone is not UTC, so GitHub
timestamp comparisons must use `START_UTC`, not the local date string.
...
const SUMMARY = { type: 'object', required: ['project', 'goal', 'narrative', 'outcome', 'ship_status'], properties: {
  project: STR, goal: STR, narrative: STR, outcome: STR,
  ship_status: { enum: ['shipped', 'merged_undeployed', 'in_pr', 'in_progress', 'abandoned', 'ops_only', 'n/a'] },
  incidents: ARR(STR), unresolved: ARR(STR) } }
const SESSIONS = { type: 'object', required: ['summaries'], properties: { summaries: ARR(SUMMARY) } }
const GIT = { type: 'object', required: ['repos'], properties: {
  repos: ARR({ type: 'object', required: ['repo'], properties: { repo: STR, branches: ARR(STR),
    commits: ARR({ type: 'object', properties: { hash: STR, subject: STR, committed_at: STR } }) } }),
  traces: ARR({ type: 'object', properties: { slug: STR, status: STR, linear: STR, latest: STR } }) } }
const LINEAR = { type: 'object', required: ['issues'], properties: {
  issues: ARR({ type: 'object', required: ['id', 'title', 'state'], properties: { id: STR, title: STR, state: STR } }), note: STR } }
...
Before writing a single line, load the `write-as-me` skill. The report is
...
8. Always load the `write-as-me` skill before composing the report (both
...
- **`tdl` not logged in**: caught in pre-flight; suggest `! tdl login`,
  re-check, otherwise skip Telegram.

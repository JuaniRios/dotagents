---
allowed-tools: Bash(staging-remote:*), Bash(prod-remote:*), Bash(sqlite3:*), Bash(tail:*), Bash(head:*), Read, Grep, Glob
description: Diagnose the liquidity bot's health (hedging, rebalancing, errors, overall status) by querying the live server directly via `prod-remote`/`staging-remote`.

...
description: Fetch the latest GitHub CI run for this branch, diagnose failures, fix them locally, and amend.
argument-hint: "[stack]"
---
...
nix builds, and anything else CI runs that `/ci` skips locally.

## Stack mode

When invoked as `/ci-fix stack`, work through the entire upstack:

1. Record the starting branch.
2. Run the normal `/ci-fix` flow on the current branch.
3. After fixes pass, if files were modified, use `/graphite` to amend
   via `gt modify -a`.
4. Use `/graphite` to `gt up`. If it succeeds, repeat from step 2.
...
Apply the same fix rules as `/ci`:
...
3. **If files were modified**, use `/graphite` to amend via `gt modify -a`.
4. Stop.
